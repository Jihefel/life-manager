  ...
  return (
    <section className='scheduler pb-6'>
      <div className={`days flex flex-col justify-center py-6 sticky top-0 z-10 ${props.theme === "dark" ? "bg-darkmode" : "bg-white"}`}>
        <ToggleButtonGroup color='primary' className='flex flex-wrap justify-center py-3' exclusive value={selectedDay} onChange={changeDay}>
          {props.joursAbreges.map((jour, index) => (
            <ToggleButton value={props.jours[index]} className='px-3' key={props.jours[index]} size='small'>
              {jour}
            </ToggleButton>
          ))}
        </ToggleButtonGroup>
      ...
      <div className='menu container mx-auto'>
        {Object.entries(menu[selectedDay][selectedVersion]).map(([periode, repas], index) => (
          <Accordion expanded={expanded === `panel${index + selectedDay}`} onChange={handleChange(`panel${index + selectedDay}`)} key={index + selectedDay}>
            <AccordionSummary expandIcon={<MdExpandMore />} aria-controls={`panel${index}bh-content`} id={`panel${index}bh-header`} className=' items-center'>
